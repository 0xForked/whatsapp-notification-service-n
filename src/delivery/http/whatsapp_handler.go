...
func NewWhatsappHandler(whatsappService domain.WhatsappService, publicRoutes, privateRoutes fiber.Router) {
	handler := &WhatsappHandler{
		WhatsappService: whatsappService,
		Validate: utils.NewValidator(),
	}

	whatsappRoute := publicRoutes.Group("/whatsapp")
	whatsappRoute.Post("/login", handler.Login)
	whatsappRoute.Get("/info", handler.GetInfo)
	whatsappRoute.Post("/send-text", handler.SendText)
	whatsappRoute.Post("/send-location", handler.SendLocation)
	whatsappRoute.Post("/send-document", handler.SendDocument)
	whatsappRoute.Post("/send-image", handler.SendImage)
	whatsappRoute.Post("/send-audio", handler.SendAudio)
	whatsappRoute.Post("/send-video", handler.SendVideo)
	whatsappRoute.Get("/groups/:jid", handler.Groups)
	whatsappRoute.Post("/logout", handler.Logout)
}
...
func (w *WhatsappHandler) Login(c *fiber.Ctx) error {
	...
	clientNameLong := c.FormValue("client_name_long", "Whatsapp REST API")
	clientNameShort := c.FormValue("client_name_short", "wagorf")
	...
	reqReconnect, err := strconv.Atoi(reconnect); if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}

	reqTimeout, err := strconv.Atoi(timeout); if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}

	reqVersionClientMajorInt, err := strconv.Atoi(reqVersionClientMajor); if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}

	reqVersionClientMinorInt, err := strconv.Atoi(reqVersionClientMinor); if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}

	reqVersionClientBuildInt, err := strconv.Atoi(reqVersionClientBuild); if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}

	qrCodeStr, err := w.WhatsappService.Login(
		reqVersionClientMajorInt,
		reqVersionClientMinorInt,
		reqVersionClientBuildInt,
		reqTimeout,
		reqReconnect,
		clientNameShort,
		clientNameLong,
...
func (w *WhatsappHandler) GetInfo(c *fiber.Ctx) error {
	info, err := w.WhatsappService.GetInfo()
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusInternalServerError, err)
	}
...
func (w *WhatsappHandler) SendText(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendText(form)
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}
...
func (w *WhatsappHandler) SendLocation(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendLocation(form)
...
func (w *WhatsappHandler) SendImage(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendFile(form, "image")
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusInternalServerError, err)
	}
...
func (w *WhatsappHandler) SendAudio(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendFile(form, "audio")
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusInternalServerError, err)
	}
...
func (w *WhatsappHandler) SendVideo(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendFile(form, "video")
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusInternalServerError, err)
	}
...
func (w *WhatsappHandler) SendDocument(c *fiber.Ctx) error {
	...
	msgId, err := w.WhatsappService.SendFile(form, "document")
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusInternalServerError, err)
	}
...
func (w *WhatsappHandler) Groups(c *fiber.Ctx) error {
	...
	groupMd, err := w.WhatsappService.Groups(jid)
	if err != nil {
		return domain.NewHttpError(c, fiber.StatusBadRequest, err)
	}
